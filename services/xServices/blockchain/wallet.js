...
export const getBalanceHistory = async (currency, xid) => {
  ...
  const { error, result } = await runMethod("balance.history", filter, "user", {
    uid: xid
  });
...
export const updateBtcBalance = async xid => {
  ...
  const unsaved = await filterUnsaved(
    "BTC",
    xid,
    confirmedTxIds,
    fetchTransaction
...
export const filterUnsaved = async (currency, xid, txids, fetchTransaction) => {
  ...
  const freshTXids = txids.filter(
    hash => !savedTXids.includes(`${currency}-${hash}`)
  );
...
export const hashToId = (hash, currency, vout = 0) => {
  ...
  const postoffset = 6;
  const postfix = (currency === "BTC" ? 0b100000 : 0b010000) + (vout % 0b10000);
